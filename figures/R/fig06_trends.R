...
nmed_all <- read_csv(file.path(ROOT, "data/processed/insitu_annual_medians.csv"),
                     show_col_types = FALSE)
# serie principal = epoca seca; la campana de lluvias (dic 2012) solo se muestra
nmed <- filter(nmed_all, season == "dry")
wet  <- filter(nmed_all, season == "wet") |> mutate(zone_label = map_strict(
  zona, c("BAHIA PUNO" = "Bahia de Puno", "LAGO MENOR" = "Lago Menor",
          "LAGO MAYOR" = "Lago Mayor"), "zona"))
...
stripes <- expand.grid(zone_label = ZONES, year = YEARS, stringsAsFactors = FALSE) |>
  ...
  left_join(wet |> select(zone_label, year, wet_median = secchi_median),
            by = c("zone_label", "year")) |>
  mutate(zl = zone_factor(zone_label),
         state = case_when(!is.na(secchi_median) ~ "data",
                           !is.na(wet_median) ~ "wet",
...
pa <- ggplot(stripes, aes(year, zl)) +
  ...
  # campana de lluvias: se ve su valor, rayado y fuera de la serie
  geom_tile(data = filter(stripes, state == "wet"), aes(fill = wet_median),
            colour = "white", linewidth = 0.6, height = 0.86, alpha = 0.45) +
  geom_segment(data = filter(stripes, state == "wet"),
               aes(x = year - 0.45, xend = year + 0.45,
                   y = 4 - as.numeric(zl) - 0.4, yend = 4 - as.numeric(zl) + 0.4),
               colour = "white", linewidth = 0.5) +
  geom_text(data = filter(stripes, state == "wet"),
            aes(label = sprintf("%.1f", wet_median)), size = 2.1,
            fontface = "italic", colour = INK) +
  annotate("text", x = 2012, y = 3.62, label = "wet season", size = 2,
  ...
  coord_cartesian(xlim = c(2010.5, 2024.5), ylim = c(0.55, 3.7), clip = "off") +
  labs(x = NULL, y = NULL) +
...
# ------------------------------------------------------------------ (c) --
reads <- nmed_all |> mutate(zl = zone_factor(zona)) |>
...
pc <- ggplot(reads, aes(factor(year), n, fill = zl)) +
  ...
  geom_col(width = 0.75) +
  annotate("text", x = 2, y = 100, label = "wet\nseason", size = 1.9,
           colour = INK_2, fontface = "italic", lineheight = 0.9) +
