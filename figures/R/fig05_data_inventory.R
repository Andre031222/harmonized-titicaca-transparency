...
ZMAP <- c("BAHIA PUNO" = "Bahia de Puno", "LAGO MENOR" = "Lago Menor",
...
pa <- ggplot(cov, aes(mes, factor(year))) +
  ...
  labs(title = "(a)  Tres años sin campaña y ni un solo dato fuera de la época seca",
       subtitle = paste("Número de match-ups satélite–campo por año y mes.",
                        "No existe dato de 2020, 2021 ni 2023,\nasí que el",
                        "hold-out «2022–2024» sólo contiene 2022 y 2024"),
       x = NULL, y = NULL) +
...
seas2 <- seas |>
  mutate(mes = factor(month_label, levels = MES),
         season = factor(season, levels = c("Lluvias (Dic-Mar)", "Transicion",
                                            "Seca (Jul-Oct)"),
                         labels = c("Lluvias (dic–mar)", "Transición",
                                    "Seca (jul–oct)")))
...
pb <- ggplot(seas2, aes(mes, n, fill = season)) +
  ...
  annotate("text", x = 3.1, y = 235, label = "0 match-ups en toda\nla época de lluvias",
           size = 2.5, colour = ACCENT, fontface = "bold", lineheight = 1.1,
           hjust = 0.5) +
  scale_fill_manual(values = c("Lluvias (dic–mar)" = "#7BA8C4",
                               "Transición" = "grey82",
                               "Seca (jul–oct)" = "#2E6E8E"), name = NULL) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.14))) +
  labs(title = "(b)  Sólo hay datos de época seca",
       subtitle = paste("Las floraciones de Bahía de Puno ocurren\nen época de",
                        "lluvias, que el dataset no\ncubre en absoluto"),
       x = NULL, y = "Match-ups") +
...
pc <- ggplot(pc_df, aes(secchi, zone_label, fill = zone_label)) +
  ...
  labs(title = "(c)  El gradiente trófico domina el Secchi",
       subtitle = paste("Sólo 73 valores distintos en 812 match-ups:\nlas",
                        "lecturas se anotan al medio metro, así que\nel RMSE de",
                        "1.86 m roza la granularidad de la\npropia medida de",
                        "referencia"),
       x = "Profundidad de disco Secchi (m)", y = NULL)
