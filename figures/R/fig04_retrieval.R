...
oof  <- oof  |> mutate(zone_label = zone_factor(zone_label))
zone <- zone |> mutate(zone_label = zone_factor(zone_label))
...
pa <- ggplot(oof, aes(secchi, predicted, colour = zone_label)) +
  ...
  labs(title = "(a)  Predicted vs measured",
...
pc <- ggplot(pc_df, aes(R2, name, fill = kind)) +
  ...
  scale_fill_manual(values = c(artefacto = NEUTRAL, correcto = "#2E6E8E"),
                    guide = "none") +
  # el limite derecho deja sitio a la etiqueta "+0.574" de la barra mas
  # larga, que con 1.25 quedaba cortada por el borde del panel
  scale_x_continuous(limits = c(-3.3, 1.75), breaks = seq(-3, 1, 1),
                     expand = expansion(mult = c(0.01, 0))) +
  labs(title = "(c)  The baseline was misimplemented",
...
pd <- art |>
  ...
  labs(title = "(d)  One impossible value ruins R²",
