...
![Lake Titicaca in 3D, with the monitoring stations coloured by measured transparency](results/figures_r/graphical_abstract.png)
...
rivers (HydroSHEDS), and the Peru–Bolivia border crossing the lake. (c) The 143 IMARPE
...
tables are typed, and `p10` checks all 135 of them against `results/tidy/` on every
